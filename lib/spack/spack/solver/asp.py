...
class AspGenerator(object):
    ...
    def pkg_version_rules(self, pkg):
        ...
        for v in pkg.versions:
            self.fact(fn.declared_version(pkg.name, v))
    ...
    def spec_versions(self, spec):
        ...
        if spec.concrete:
            ...
            if spec.versions.concrete and spec.version not in versions:
                self.fact(fn.declared_version(spec.name, spec.version))
                versions.append(spec.version)

            # disallow any versions that do not satisfy the spec
            ...
            for v in versions:
                if not v.satisfies(spec.versions):
                    self.fact(fn.disallowed_version(spec.name, v))
